...
Systematic hyperparameter search across 12 configurations (k ∈ {2, 4, 8}, chunk_size ∈ {200, 300, 400, 500}) using a custom LLM-as-judge evaluation framework with four metrics:
...
| + Query Rewriting | 0.5545 | Dropped — rejected |
| + MMR Retrieval | 0.7273 | Improved — kept |
| + Prompt V2 | 0.7727 | Best overall |
| + Hybrid Search (0.5/0.5) | 0.7273 | Dropped — rejected |
| + Hybrid Search (0.7/0.3) | 0.7273 | No improvement — rejected |

**Key findings:**
- Larger chunks (500) consistently outperform smaller chunks across all k values — preserves semantic context better for these document types
- k=4 balances retrieval coverage without adding noise; k=2 consistently underperforms, k=8 adds marginal recall improvement at cost of precision
- Query rewriting hurt completeness on factual Q&A — raw questions are already precise search queries for this domain
- MMR improved completeness by reducing redundant chunk retrieval
- Hybrid search (BM25 + semantic) added noise for this dataset — rejected
- LLM-as-judge evaluation requires a completeness metric to avoid inflated scores from "I don't know" responses being rated as faithful
...
| URL | Web pages; HTML is parsed and cleaned automatically |

---

## Known Limitations

- Scanned/handwritten PDFs are not supported — text extraction requires OCR
- Context recall drops for very specific factual queries (dates, names) due to embedding model limitations
- Groq free tier has rate limits; sustained heavy usage may hit limits
- ChromaDB persists to disk — clearing session removes all indexed documents
...
- Chunk size affects answer quality more than retrieval depth (k) for this document type
...
- [Energy Consumption Forecasting](github link) — LSTM time series forecasting with FastAPI + Docker
